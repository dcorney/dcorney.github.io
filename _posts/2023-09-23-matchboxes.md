...
|                                                                                                                                                                             |                                                                                                                                                     |
| :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------- |
| <img src="/images/matchboxes/contents_green.JPG" alt="a matchbox with the word 'green' written on top, open to show the word eggs on multiple slips of paper" width="300"/> | The box **green** only contains the word 'eggs' over and over again. This is because in our text, the word "green" is always followed by "eggs".    |
...
One of the boxes is specially marked **\<start\>** and the slips of paper in this box show every word that can start a sentence, based on the training text. To generate a new sentence, we simply take a random slip of paper from this box and note the word. This is the first word of the new sentence. To generate each successive word, we open the box labelled with the current word and pick one slip at random and read the word. This continues until the new word has a terminating punctuation mark (i.e. a full-stop, exclamation mark or question mark). 
...
It's not going to win a [Geisel Award](https://www.ala.org/alsc/awardsgrants/bookmedia/geisel), but it is a proper sentence, it was written by MESLaM and it doesn't appear anywhere in the training text.

Here's a video of MESLaM producing exactly that sentence:

{% include youtube.html id="KKVvI-7yiSA" %}


Each time we run the process, we'll generate a new sentence driven by the random selections. Here's another video of MESLaM generating the sentence ["Could not like them anywhere."](https://youtu.be/er3N7RB5-4Q).
